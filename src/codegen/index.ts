import * as es from 'estree'
import * as l from 'llvm-node'
import { Environment, Type, TypeRecord } from '../context/environment'

class ProgramExpression {
  static codegen(
    node: es.Program,
    env: Environment,
    context: l.LLVMContext,
    module: l.Module
  ): any {
    const body: Array<es.Directive | es.Statement | es.ModuleDeclaration> = node.body
    body.forEach(x => evaluate(x, env, context, module))
  }
}
class VariableDeclarationExpression {
  static codegen(
    node: es.VariableDeclaration,
    env: Environment,
    context: l.LLVMContext,
    module: l.Module
  ) {
    const kind = node.kind
    if (kind !== 'const') {
      Error('We can only do const right now')
    }
    const decl = node.declarations[0]
    const id = decl.id
    const init = decl.init
    let name: string | undefined
    let value: any
    let raw: string | undefined
    let typerecord: TypeRecord
    if (id.type === 'Identifier') {
      name = id.name
    }
    if (init && init.type === 'Literal') {
      value = init.value
      raw = init.raw
    }
    if (!value || !name) {
      Error('Something wrong with the literal\n' + JSON.stringify(node, null, 2))
      return
    }
    let initializer
    let globalVariable
    switch (typeof value) {
      case 'string':
        const len = value.length
        const arrayType = l.ArrayType.get(l.Type.getInt32Ty(context), len)
        const elements = Array.from(value).map(x => l.ConstantInt.get(context, x.charCodeAt(0)))
        initializer = l.ConstantArray.get(arrayType, elements)
        globalVariable = new l.GlobalVariable(
          module,
          arrayType,
          true,
          l.LinkageTypes.InternalLinkage,
          initializer,
          name
        )
        typerecord = { type: Type.STRING }
        break
      case 'number':
        const doubleType = l.Type.getDoubleTy(context)
        initializer = l.ConstantFP.get(context, value)
        globalVariable = new l.GlobalVariable(
          module,
          doubleType,
          true,
          l.LinkageTypes.InternalLinkage,
          initializer,
          name
        )
        typerecord = { type: Type.NUMBER }
        break
      case 'boolean':
        const intType = l.Type.getInt32Ty(context)
        initializer = value ? l.ConstantInt.getTrue(context) : l.ConstantInt.getFalse(context)
        globalVariable = new l.GlobalVariable(
          module,
          intType,
          true,
          l.LinkageTypes.InternalLinkage,
          initializer,
          name
        )
        typerecord = { type: Type.BOOLEAN }
        break
      default:
        Error('Unrecognized datatype')
        return
    }
    env.push(name, typerecord)
  }
}

const jumpTable: { [id: string]: any } = {
  Program: ProgramExpression,
  VariableDeclaration: VariableDeclarationExpression
}

function evaluate(node: es.Node, env: Environment, context: l.LLVMContext, module: l.Module) {
  const fun = jumpTable[node.type]
  if (fun) {
    fun.codegen(node, env, context, module)
  } else {
    Error('Unknown node ' + node.type)
  }
}

function eval_toplevel(node: es.Node) {
  const context = new l.LLVMContext()
  const module = new l.Module('module', context)
  const env = new Environment(new Map<string, TypeRecord>(), undefined)
  evaluate(node, env, context, module)
  return module
}

export { eval_toplevel }
