...
function findAndMarkTailCalls(expr: es.Expression, currentFunctionName: string): boolean {
  if (expr.type === 'CallExpression') {
    ...
    if (call.callee.type === 'Identifier' && (call.callee as es.Identifier).name === currentFunctionName) {
    ...
    return findAndMarkTailCalls(tenary.consequent, currentFunctionName) || findAndMarkTailCalls(tenary.alternate, currentFunctionName)
  }
